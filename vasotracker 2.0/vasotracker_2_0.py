...
class AcquisitionSettingsPane(ToolbarPane):
    def __init__(self, parent, model_vars: VtState):
        ...
        # Create a tooltip for each row
        self.camera_tooltip = ToolTip(self)
        self.scale_tooltip = ToolTip(self)
        self.exp_tooltip = ToolTip(self)
        self.acq_rate_tooltip = ToolTip(self)
        self.rec_intvl_tooltip = ToolTip(self)
        self.adjust_settings_tooltip = ToolTip(self)

        # Assign tooltips to rows
        self.bind_tooltip_to_row_widgets(0, self.camera_tooltip, "Select the camera input.")
        self.bind_tooltip_to_row_widgets(1, self.scale_tooltip, "Set the scale in micrometers per pixel.")
        self.bind_tooltip_to_row_widgets(2, self.exp_tooltip, "Set the camera exposure.")
        self.bind_tooltip_to_row_widgets(4, self.acq_rate_tooltip, "Current acquisition rate (Hz).")
        self.bind_tooltip_to_row_widgets(5, self.rec_intvl_tooltip, "Number of seconds between saved images.")
        self.bind_tooltip_to_row_widgets(6, self.adjust_settings_tooltip, "Enable/disable default settings.")

    def bind_tooltip_to_row_widgets(self, row, tooltip_instance, text):
        for child in self.winfo_children():
            grid_info = child.grid_info()
            # Check if the widget is placed in the correct row
            if grid_info.get("row") == row:
                child.bind("<Enter>", lambda e: tooltip_instance.showtip(text))
                child.bind("<Leave>", lambda e: tooltip_instance.hidetip())
...
class AnalysisSettingsPane(ToolbarPane):
    def __init__(self, parent, model_vars: VtState):
        ...
        # Create a tooltip for each row
        self.lines_tooltip = ToolTip(self)
        self.smooth_tooltip = ToolTip(self)
        self.integration_tooltip = ToolTip(self)
        self.threshold_tooltip = ToolTip(self)
        self.filter_tooltip = ToolTip(self)
        self.ID_tooltip = ToolTip(self)
        self.rotate_tooltip = ToolTip(self)

        # Assign tooltips to rows
        self.bind_tooltip_to_row_widgets(0, self.lines_tooltip, "Select the number of lines to track.")
        self.bind_tooltip_to_row_widgets(1, self.smooth_tooltip, "Width (in pixels) of smoothing window.")
        self.bind_tooltip_to_row_widgets(2, self.integration_tooltip, "Set how many rows of pixels are used for each line profile.")
        self.bind_tooltip_to_row_widgets(3, self.threshold_tooltip, "Set threshold for identfying outliers.")
        self.bind_tooltip_to_row_widgets(4, self.filter_tooltip, "Enable/disable outlier detection.")
        self.bind_tooltip_to_row_widgets(5, self.ID_tooltip, "Enable/disable inner diameter or fluorescence mode.")
        self.bind_tooltip_to_row_widgets(6, self.rotate_tooltip, "Change horizontal/vertical tracking.")
...
class CaliperROIPane(ToolbarPane):
    def __init__(self, parent, model_vars: VtState):
        ...
        ttk.Label(self, text="Multi:").grid(row=2, column=0, columnspan=2, sticky=tk.EW)
...
class DataAcquisitionPane(ToolbarPane):
    def __init__(self, parent, model_vars: VtState):
        ...
        ttk.Label(self, text="Time (s):", anchor="center").grid(row=2, column=3, padx=(5, 20), pady=5, sticky=tk.EW)
...
class PressureControlPane(ToolbarPane):
    def __init__(self, parent, model_vars: VtState):
        ...
        # Create a tooltip for each row
        self.pressure_tooltip = ToolTip(self)
        self.scale_tooltip = ToolTip(self)
        self.exp_tooltip = ToolTip(self)
        self.acq_rate_tooltip = ToolTip(self)
        self.rec_intvl_tooltip = ToolTip(self)
        ...
        # Assign tooltips to rows
        self.bind_tooltip_to_row_widgets(1, self.pressure_tooltip, "Set the desired pressure.")
        self.bind_tooltip_to_row_widgets(2, self.scale_tooltip, "Set the pressure increment.")
        self.bind_tooltip_to_row_widgets(2, self.exp_tooltip, "Set the camera exposure.")
        self.bind_tooltip_to_row_widgets(4, self.acq_rate_tooltip, "Current acquisition rate (Hz).")
        self.bind_tooltip_to_row_widgets(5, self.rec_intvl_tooltip, "Number of seconds between saved images.")
        self.bind_tooltip_to_row_widgets(6, self.adjust_settings_tooltip, "Enable/disable default settings.")
...
class View(ttk.Frame):
    def __init__(
        ...
        self.grid_rowconfigure(0, weight=2, uniform="row")
        ...
        def set_toolbar_min_height():
            self.grid_rowconfigure(
                0, weight=2, minsize=self.toolbar.winfo_height() + 50, uniform="row"
            )
