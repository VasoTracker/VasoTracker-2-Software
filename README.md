...
* **FMD / vascular-ultrasound detection mode.** A new "FMD" checkbox in Analysis settings switches to a B-mode wall model built for flow-mediated dilation: it locates the bright near/far wall reflections either side of the anechoic lumen, reports **inner diameter = the lumen (intima-intima)** and outer = wall-to-wall, averages along the vessel to beat speckle, and seeds each frame from the last so the vessel can drift without the ROI clipping it. On test clips it recovers the dilation curve the previous ultrasound mode missed entirely.
